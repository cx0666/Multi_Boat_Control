...
class StateEstimatorNode:
    ...
    def rtk_callback(self, msg):
        """
        Handle RTK GPS data messages and output fused information.
        ...
        with self.data_lock:
            ...
            # 立即输出融合的RTK和Pixhawk航向角信息
            self.output_fused_rtk_data(msg)
            
            rospy.logdebug("RTK data received: lat=%.6f, lon=%.6f, speed=%.2f m/s", 
                          msg.latitude_deg, msg.longitude_deg, msg.ground_speed_mps)
    ...
    def output_fused_rtk_data(self, rtk_msg):
        """
        Output fused RTK and Pixhawk heading data.
        
        Args:
            rtk_msg (RTK): Current RTK message
        """
        # 简化逻辑：优先保证数据正常输出，取消严格的时间同步检测
        heading_value = self.latest_heading if self.latest_heading is not None else 0.0
        
        # 输出融合信息（按您要求的格式）
        rospy.loginfo(f"Published RTK data: Lat={rtk_msg.latitude_deg:.7f}, "
                     f"Lon={rtk_msg.longitude_deg:.7f}, "
                     f"Speed={rtk_msg.ground_speed_mps:.3f} m/s, "
                     f"Pixhawk航向角: {heading_value:.1f}°")
    ...
    def check_data_quality(self):
        ...
        if self.latest_rtk and self.rtk_timestamp:
            ...
            rtk_valid = (age < self.max_data_age)  # 简化：只检查时间，不检查fix状态
    ...
    def create_boat_state_message(self):
        ...
        with self.data_lock:
            ...
            if rtk_valid and self.latest_rtk:
                state_msg.latitude = self.latest_rtk.latitude_deg
                state_msg.longitude = self.latest_rtk.longitude_deg
                ...
                state_msg.gps_fix_valid = True  # 简化：假设RTK数据有效
